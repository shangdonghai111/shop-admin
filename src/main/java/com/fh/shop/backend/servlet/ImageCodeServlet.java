...
public class ImageCodeServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//request.getSession().setAttribute(Gloable.SYSTEM_IMAGECODE, scode);
		String fh_id = CookieUtil.readCookie(request, "fh_id");
		String uuid = UUID.randomUUID().toString().replace("-", "").toUpperCase();
		if (StringUtils.isEmpty(fh_id)){
			RedisUtil.set("code"+uuid,scode);//开辟空间 关联
			RedisUtil.expire("code"+uuid,5*60);//设置过期时间
			//响应给客户端
			CookieUtil.writeCookie(response,"fh_id",uuid,"admin.fh.com",-1);
		}else {
			RedisUtil.set("code"+fh_id,scode);
			RedisUtil.expire("code"+uuid,5*60);//设置过期时间
		}
		// 输出图像
		try {
